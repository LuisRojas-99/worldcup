...
echo  "$($PSQL" SELECT name FROM teams JOIN games ON teams.team_id = games.winner_id or teams.team_id = games.opponent_id WHERE year = 2014 Group BY NAME; ")"
...
echo "$($PSQL" SELECT games.year, teams.name FROM games JOIN teams ON teams.team_id = games.winner_id WHERE games.round = 'Final' ORDER BY year; ")"
